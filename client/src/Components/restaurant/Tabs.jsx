...
const Tab = (props) => {
    ...
    return (
        <>  <Link to={`/restaurant/${id}/${props.route}`}>
            ...
            <div className={classnames("text-gray-500 font-medium ", {
                "text-zomato-400 font-semibold border-b-2 border-zomato-400": props.isActive,
                }) }>
                <h3 className="text-lg md:text-xl font-light">{props.title}</h3>
            </div>
            </Link>
        </>
    );
...
const TabContainer = (props) => {
    ...
    return (
        <>
            <div className="flex items-center pb-4 gap-10 md:gap-20 overflow-x-scroll border-b-2">
              {tabs.map((tab) => (
